...
def quotechar_split(s, qc='"', sep=' '):
    """
    String split with quotecharacter qc: behave similary to like the usual python 
    s.split(sep), except that occurrences of sep between qc's are not counted.
    
    Parameters
    ----------
    s: string
    qc: string, the quote character
    setp: string, the seperator
    Example:
    quotechar_split('my,name",is",what', qc='"', sep=',') = ['my', 'name",is"', 'what']

    If there are an odd number of occurrences of qc, everything after the last one is
    considered to be part of a quotation.
    """

    assert len(qc)==1, 'quoate character length not equal to one'

    split_s = s.split(sep)
    qc_bool = [qc not in entry for entry in split_s]
    ret = []
    i=0
    while i < len(split_s):
        if qc_bool[i]:
            ret.append(split_s[i])
            i+=1
        else:
            try:
                if qc_bool[i+1]:
                    ret.append(sep.join([split_s[i], split_s[i+1]]))
                i+=2
            except IndexError:
                break
